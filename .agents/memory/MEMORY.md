...
- [Donut narrow-segment hit targets](donut-narrow-segment-hit-target.md) — thin SVG donut slices need a separate touch target from their visible fill
...
- [Splash data readiness gate](splash-data-gate.md) — start the initial home prefetch under the intro and gate sniff/lick until household-dependent data has settled
...
- [Custom Larder buckets](custom-larder-buckets.md) — scope definitions by user/household, keep stable keys on rename, and localize only untouched defaults
