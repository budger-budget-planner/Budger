...
BASE = Path(__file__).with_name("generate_appstore_readiness_pdf.py")
spec = importlib.util.spec_from_file_location("pdf_base", BASE)
if spec is None or spec.loader is None:
    raise RuntimeError("Could not load PDF base module")
module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)
Pdf = module.Pdf

OUT = Path("attached_assets/budger-google-play-readiness-audit.pdf")


def make_document() -> Pdf:
    p = Pdf()
    p.start_page()
    p.text("Google Play Store readiness", 48, p.y, 25, "F2", (0.04, 0.10, 0.18))
    p.y -= 28
    p.text("Budger  |  Read-only technical audit", 48, p.y, 11, "F1", (0.38, 0.43, 0.50))
    p.y -= 25
    p.callout(
    ...
    p.heading("Current Android/native project state", 1)
    p.para("The repository search found no Android project files. There is no AndroidManifest.xml, Gradle project, Gradle wrapper, package.json for a native app, google-services.json, EAS configuration, or Capacitor configuration.")
    p.para("The only Android-specific configuration is an android section inside export/ios/app.config.ts. That file is explicitly described as an iOS-focused Expo configuration and references assets that are not present.")
    p.para("The current configuration references:")
    p.code("export/ios/assets/adaptive-icon.png")
    p.para("That asset is missing. There are also no Android launcher icon sets, splash assets, adaptive icon XML files, or native resource directories.")
    p.para("The configuration contains a placeholder EAS project identifier:")
    p.code("REPLACE_WITH_EAS_PROJECT_ID")
    p.para("This means the current Android material is a preparation template, not a project that can be opened in Android Studio or uploaded to Google Play Console.")

    p.heading("What is required before a Play upload", 1)
    for item in [
    ...
    p.heading("Important technical concern: the product is currently web-first", 1)
    p.para("The application currently relies on React/Vite, browser localStorage, browser service workers, browser Notification, browser PushManager, VAPID Web Push, and browser camera/file APIs.")
    p.para("A basic WebView wrapper would not automatically provide:")
    for item in [
        ...
        p.bullet(item)
    p.para("The current frontend registers a browser service worker at /sw.js and uses PushManager with a VAPID key. That is a web-push design, not an Android FCM design. A native Android wrapper would need an intentional push architecture and backend token registration path.")
    ...
    p.heading("Android permissions and declarations needing verification", 1)
    p.para("The current Android section declares:")
    for item in [
        ...
        p.bullet(item)
    p.para("These declarations are not backed by a compiled native project yet. Before submission:")
    for item in [
    ...
    p.heading("Google Play target API and packaging", 1)
    p.para("Google's current target-SDK guidance says that, starting August 31, 2026, new apps and app updates must target Android 16 (API level 36) or higher, with limited platform-specific exceptions. For a release planned before that date, the applicable current Play requirement must be checked again at upload time.")
    p.para("Google Play uses the Android App Bundle format for distribution. The deliverable must therefore be a signed .aab produced from the native Android build, with Play App Signing configured. The current repository contains neither an Android build nor an AAB.")
    p.para("The final release should also be checked for 64-bit support, modern SDK compatibility, dependency security, network security, backup behavior, deep links, and correct handling of Android process death and rotation.")

    p.heading("Data Safety and privacy status", 1)
    p.para("The privacy foundation is promising but the Play Console declarations have not been completed or verified against a native dependency tree.")
    p.para("Likely data categories requiring review include:")
    for item in [
        ...
        p.bullet(item)
    p.para("Before submission, the owner must confirm:")
    for item in [
    ...
    p.heading("Account deletion and user access", 1)
    p.para("The app already has an account-deletion request flow and a data-export endpoint, which is a strong starting point.")
    p.para("For Google Play, verify the complete submitted-app flow:")
    for item in [
    ...
    p.heading("Monetization and policy checks", 1)
    p.para("No Google Play Billing implementation was found in the audited source. If Budger later sells digital subscriptions, premium features, or other in-app digital goods, Google Play Billing and the relevant payments policy must be evaluated before release.")
    p.para("If the app remains free with no ads and no digital purchases, the store listing and Data Safety answers should say so accurately. Do not add billing, advertising, or analytics declarations unless those SDKs and flows are actually present.")

    p.heading("Testing and release checklist", 1)
    for item in [
    ...
    p.para("Once the native approach is selected - Expo/React Native, Capacitor, or another intentional Android strategy - the project can be prepared with native permissions, push registration, Android assets, a reproducible signed AAB build, Play Console metadata, and release testing.")

    p.heading("Official Google reference basis", 1)
    for item in [
