      ...
      <LarderStackMotionContext.Provider value={motion}>
        <div ref={ref} className={`relative ${className}`}>
          ...
          <div className={`relative z-10 grid ${phase === "crossfade" ? "larder-stack-motion-frame" : ""}`}>
            <div className={phase === "crossfade" ? "larder-stack-outgoing-clip pointer-events-none" : "larder-stack-primary-frame"}>
            ...
            {phase === "crossfade" && (
              <LarderStackMotionContext.Provider value={{ ...motion, incomingSurface: true }}>
                <div aria-hidden="true" className="larder-stack-incoming-frame larder-stack-fade-in pointer-events-none">
                  {children}
                </div>
    ...
    <>
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          {breakdown.filter(asset => Math.abs(asset.rawTotal) >= 0.005).map(asset => (
        ...
        <span className="mt-0.5 flex items-center gap-1 text-[9px] font-medium text-white/40">
          {t("larder.flip_stack")} <ArrowRight className="h-3 w-3" />
        </span>
      </div>
    ...
    <div className={`relative grid ${className}`}>
      <style>{`
        @keyframes larderStackShuffleOut {
          ...
          18% { opacity: 1; transform: translate3d(24px, -1px, 0) rotate(3deg) scale(.99); }
          48% { opacity: .92; transform: translate3d(112px, -7px, 0) rotate(8deg) scale(.97); }
          68% { opacity: 1; transform: translate3d(210px, -4px, 0) rotate(11deg) scale(.95); }
          78% { opacity: .86; transform: translate3d(250px, -1px, 0) rotate(12deg) scale(.94); }
          100% { opacity: 0; transform: translate3d(118%, -4px, 0) rotate(14deg) scale(.9); }
        }
        .larder-stack-shuffle-out { transform-origin: 50% 50%; will-change: transform, opacity; animation: larderStackShuffleOut 400ms cubic-bezier(.22, .72, .34, 1) both; }
        .larder-stack-motion-frame { grid-template-columns: minmax(0, 1fr); }
        .larder-stack-primary-frame { position: relative; }
        .larder-stack-outgoing-clip { position: absolute; top: 0; left: 0; width: 200%; height: 100%; z-index: 1; overflow: hidden; }
        .larder-stack-outgoing-frame { width: 50%; }
        .larder-stack-incoming-frame { position: relative; z-index: 2; }
        @keyframes larderStackFadeIn { 0% { opacity: .16; } 100% { opacity: 1; } }
        .larder-stack-fade-in { will-change: opacity; animation: larderStackFadeIn 520ms ease-out both; }
        @keyframes savingsBucketOutgoing { 0% { opacity: 1; transform: translate3d(0, 0, 0); } 72% { opacity: .9; transform: translate3d(72px, -2px, 0); } 100% { opacity: 0; transform: translate3d(118%, -3px, 0); } }
        @keyframes savingsBucketIncoming { 0% { opacity: .16; } 100% { opacity: 1; } }
        .savings-bucket-outgoing { animation: savingsBucketOutgoing 420ms cubic-bezier(.22, .72, .34, 1) both; }
        .savings-bucket-incoming { animation: savingsBucketIncoming 520ms ease-out both; }
        @media (prefers-reduced-motion: reduce) {
          .larder-stack-shuffle-out, .larder-stack-fade-in, .savings-bucket-outgoing, .savings-bucket-incoming { animation-duration: 1ms; }
        }
      `}</style>
      <button
        type="button"
      ...
      >
        {stackMotion ? bucketContent : (
          <>
            <div className={`col-start-1 row-start-1 ${transitionBucket ? "savings-bucket-outgoing pointer-events-none" : ""}`}>
