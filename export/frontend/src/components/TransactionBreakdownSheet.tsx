...
export function TransactionBreakdownSheet({ tx, categories, open, isOnline, onClose, onSuccess, onFailure }: Props) {
  ...
  const allocation = useMemo(() => {
    ...
    const invalidRows = rows.some((row, index) =>
      !row.description.trim() || parsed[index] === null,
    );
    return {
      parsed,
      allocatedCents,
  ...
  function updateRow(id: string, patch: Partial<BreakdownRow>) {
    setRows(current => {
      ...
      if (changedAmount && isLast) {
        next[changedIndex]!.amountAutoFilled = false;
      }

      // An untouched final row always represents the current remainder. This
      // keeps the fast path fast without overwriting a field the user cleared.
  ...
  function addRow() {
    setRows(current => {
      ...
      return [...current, makeRow(
        current.length,
        "",
        formatAmount(Math.max(0, sourceCents - allocatedCents)),
        "none",
        true,
      )];
    });
    ...
    <Dialog open={open} onOpenChange={value => !value && onClose()}>
      <DialogContent className="max-w-lg max-h-[92vh] overflow-y-auto">
        <DialogHeader className="pr-7">
          ...
          <DialogDescription>
            {t("breakdown.original", {
              desc: tx.description,
              date: fmtBreakdownDate(tx.date),
              amount: centsLabel(sourceCents, currency),
            })}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={submit} className="space-y-4">
                ...
                <div key={row.id} className="rounded-2xl border border-white/10 bg-card p-3 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    {rows.length > 2 && (
                      <button
                        type="button"
                        aria-label={t("breakdown.remove_row")}
                        onClick={() => removeRow(row.id)}
                        className="inline-flex items-center gap-1 rounded-lg px-2 py-1 text-xs text-white/45 hover:text-white active:bg-white/10"
                      >
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ref={index === 0 ? firstDescriptionRef : undefined}
                      id={`${row.id}-description`}
                      value={row.description}
                      onChange={event => updateRow(row.id, { description: event.target.value })}
                      placeholder={t("breakdown.name_placeholder")}
                      aria-invalid={!!error && !row.description.trim()}
                      autoComplete="off"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-[1fr_1.2fr] gap-2">
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        inputMode="decimal"
                        placeholder="0.00"
                        aria-invalid={!!error && (!!row.amount.trim() || !row.description.trim())}
                        className={allocation.overCents > 0 ? "border-amber-500/70 focus-visible:ring-amber-500/50" : ""}
                      />
                      <p className="text-[11px] text-white/40">
                        {t("breakdown.max_amount", { amount: centsLabel(maxCents, currency) })}
                      </p>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Select
                        ...
                        onValueChange={categoryId => updateRow(row.id, { categoryId })}
                      >
                        <SelectTrigger id={`${row.id}-category`} aria-label={t("breakdown.category")}>
                        ...
                        <SelectContent>
                          ...
                          {categories.map(category => (
                            <SelectItem key={category.id} value={String(category.id)}>
                              <span className="inline-flex items-center gap-2 max-w-[190px]">
          ...
          <button
            type="button"
            onClick={addRow}
            className="w-full inline-flex items-center justify-center gap-2 rounded-xl border border-dashed border-white/20 py-2.5 text-sm text-white/65 hover:text-white hover:border-white/40 active:bg-white/10"
          >
          ...
          <div className={`rounded-2xl border px-3 py-3 space-y-1 ${
            ...
            {allocation.overCents > 0 ? (
              <p className="text-xs text-amber-300" role="alert">
                {t("breakdown.over", { amount: centsLabel(allocation.overCents, currency) })}
              </p>
            ) : allocation.remainingCents > 0 ? (
              <p className="text-xs text-white/50">
                {t("breakdown.remaining", { amount: centsLabel(allocation.remainingCents, currency) })}
              </p>
            ) : null}
          </div>

          {!isOnline && (
            <p className="text-xs text-amber-300" role="alert">{t("breakdown.offline")}</p>
          )}
          ...
          <div className="flex gap-2 pt-1">
            ...
            <Button
              type="submit"
              className="flex-1"
              disabled={!isOnline || breakdown.isPending || allocation.invalidRows || !allocation.exactlyAllocated}
            >
...
function fmtBreakdownDate(date: string): string {
  return date;
}
