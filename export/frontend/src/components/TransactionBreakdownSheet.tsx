...
export function TransactionBreakdownSheet({
  tx,
  categories,
  accountCurrency,
  open,
  ...
  const [rows, setRows] = useState<BreakdownRow[]>(() => [makeRow(0), makeRow(1)]);
  ...
  const allocation = useMemo(() => {
    ...
    return {
      ...
      invalidRows: rows.some((row, index) => !row.description.trim() || parsed[index] === null),
  ...
  function fieldError(row: BreakdownRow, index: number): string | null {
    ...
    if (allocation.parsed[index] === null) return t("breakdown.amount_invalid");
    return null;
  }
    ...
    <Dialog open={open} onOpenChange={value => !value && onClose()}>
      ...
      >
        ...
        <form onSubmit={submit} className="space-y-4">
                  ...
                  <div className="grid grid-cols-[1fr_1.2fr] gap-2">
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        placeholder="0.00"
                        aria-invalid={!!error}
                        className={allocation.overCents > 0 ? "border-red-500/70 focus-visible:ring-red-500/50" : ""}
                      />
          ...
          <button
            type="button"
            onClick={() => setRows(current => [...current, makeRow(current.length)])}
          ...
          <div className={`rounded-2xl border px-3 py-3 space-y-1 ${
            ...
            {allocation.overCents > 0
              ? <p className="text-xs text-red-300" role="alert">{t("breakdown.over", { amount: centsLabel(allocation.overCents, currency) })}</p>
              : allocation.remainingCents > 0
              ? <p className="text-xs text-amber-300" role="status">{t("breakdown.remaining", { amount: centsLabel(allocation.remainingCents, currency) })}</p>
              : null}
          </div>
