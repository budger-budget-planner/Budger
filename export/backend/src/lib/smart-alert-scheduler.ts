...
async function checkBudgetAlerts(userId: number, currency: string, language: string): Promise<void> {
  const mk = monthKey();
  ...
  for (const [key, entry] of grouped) {
    ...
    if (pct >= 90) {
      ...
      if (inserted) await sendPushToUser(userId, { title: "", body: language === "pl" ? bodyPl : bodyEn, url: "/", tag: `budget-90-${key}` });
    } else if (pct >= 75) {
      ...
      if (inserted) await sendPushToUser(userId, { title: "", body: language === "pl" ? bodyPl : bodyEn, url: "/", tag: `budget-75-${key}` });
    }
  }
  ...
  if (totalPct >= 90) {
    ...
    if (inserted) await sendPushToUser(userId, { title: "", body: language === "pl" ? bodyPl : bodyEn, url: "/", tag: "budget-90-total" });
  } else if (totalPct >= 75) {
    ...
    if (inserted) await sendPushToUser(userId, { title: "", body: language === "pl" ? bodyPl : bodyEn, url: "/", tag: "budget-75-total" });
  }
}
...
async function checkGoalAlerts(
  ...
  if (activeGoals.length > 1) {
    ...
    if (inserted) await sendPushToUser(userId, { title: "", body: language === "pl" ? bodyPl : bodyEn, url: "/", tag: "goal-checkin" });
    return;
  }
  ...
  if (goal.divideByMonths) {
    ...
    if (inserted) await sendPushToUser(userId, { title: "", body: language === "pl" ? bodyPl : bodyEn, url: "/", tag: `goal-monthly-${goal.id}` });
  } else {
    ...
    if (inserted) await sendPushToUser(userId, { title: "", body: language === "pl" ? bodyPl : bodyEn, url: "/", tag: `goal-overall-${goal.id}` });
  }
}
...
export async function runSmartAlerts(): Promise<void> {
  ...
  const users = await db
    .select({ id: usersTable.id, currency: usersTable.currency, householdId: usersTable.householdId, language: usersTable.language })
    .from(usersTable)
  ...
  for (const user of users) {
    ...
    try {
      await checkBudgetAlerts(user.id, currency, language);
    } catch (err) {
      logger.warn({ err, userId: user.id }, "smart-alerts: budget check error");
    }

    try {
      await checkGoalAlerts(user.id, householdId, currency, language);
    } catch (err) {
