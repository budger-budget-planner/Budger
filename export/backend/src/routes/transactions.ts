...
// phase 0 done: breakdown is online-only and limited to ordinary user-owned
// expenses. Active splits, recurring-generated rows, Larder flows, realized-goal
// funding, and unsupported currencies are intentionally rejected. Foreign and
// currency-locked rows keep their source currency metadata. The source receipt
// is discarded with the deleted source row rather than copied to a replacement.
// phase 0 done
...
class BreakdownRequestError extends Error {
  ...
  constructor(code: string, status = 400) {
    super(code);
    this.name = "BreakdownRequestError";
    this.code = code;
    this.status = status;
  }
}
...
router.post("/transactions/:id/breakdown", async (req, res): Promise<void> => {
  ...
  try {
    ...
    const created = await db.transaction(async (dbTx) => {
      // Lock the current source row before checking eligibility and amount. This
      // prevents a concurrent edit/delete from producing a partial replacement.
      ...
      if (sourceCents === null) throw new BreakdownRequestError("breakdown_source_invalid");
      ...
      if (totalCents !== sourceCents) {
        throw new BreakdownRequestError("breakdown_total_mismatch");
      }
      ...
      // Match the normal delete path: dependent accounting rows are removed
      // before the source, while receipt rows disappear through FK cascade.
    ...
    const categoryIdsForCreated = [...new Set(created.map(tx => tx.categoryId).filter((id): id is number => id !== null))];
    ...
    const enriched = await Promise.all(created.map(tx =>
    ...
    for (const row of parsed.data.rows) {
      if (row.categoryId !== null) {
        try {
          await recordMerchantAssignment(userId, row.description, row.categoryId);
        } catch (err) {
