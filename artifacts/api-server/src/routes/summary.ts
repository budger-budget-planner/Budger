...
async function getSpendingGrouped(userId: number, filterFn?: (t: any) => boolean) {
  ...
  const unlocked = txs.filter(tx => !tx.currencyLocked && !tx.currencyUnavailable);
...
router.get("/summary/monthly", async (req, res): Promise<void> => {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const total = monthTxs.filter(t => !t.currencyLocked && !t.currencyUnavailable).reduce((s, t) => s + parseFloat(t.amount), 0);
