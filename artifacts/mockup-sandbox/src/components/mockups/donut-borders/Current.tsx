// Current border thickness: red=1.5, orange=1.5
...
// Three segments: normal (blue), over-budget (red border), stretched (orange border)
...
function Segment({ seg }: { seg: typeof segs[0] }) {
  ...
  if (isOver) {
    // over-budget: single solid fill at budget fraction then border
    return (
      <>
  ...
  // Split into spent + remain sub-arcs
  ...
  return (
    <div style={{
      ...
      <svg width={240} height={240} viewBox="0 0 320 320" style={{ overflow: "visible", display: "block" }}>
        <defs>
          <filter id="redGlow-curr" x="-25%" y="-25%" width="150%" height="150%">
        ...
        {segs.map((seg, i) => (
          <g key={i} style={{ filter: seg.label === "Over-budget" ? "url(#redGlow-curr)" : "none" }}>
            <Segment seg={seg} />
          </g>
        ))}
        {/* Centre label */}
